...
TEMPLATE_TEST_CASE("Poly", "[template]", float, double) {
  ...
  SECTION("Addition") {
    Poly<TestType> sum(p1 + p2);
    typename Poly<TestType>::Vector sol_sum(3); sol_sum << 1.0, -2.0, 3.0;
  ...
  SECTION("Subtraction") {
    Poly<TestType> sub(p1 - p2);
    typename Poly<TestType>::Vector sol_sub(3); sol_sub << 1.0, -4.0, 1.0;
  ...
  SECTION("Multiplication") {
    Poly<TestType> mul(p1 * p2);
    typename Poly<TestType>::Vector sol_mul(5); sol_mul << 0.0, 1.0, -2.0, -1.0, 2.0;
  ...
  SECTION("Differentiation") {
    ...
    typename Poly<TestType>::Vector sol_dif(2); sol_dif << -3.0, 4.0;
  ...
  SECTION("Integration") {
    ...
    typename Poly<TestType>::Vector sol_integ(4); sol_integ << 0.0, 1.0, -3.0/2.0, 2.0/3.0;
  ...
  SECTION("Integration (given constant)") {
    ...
    typename Poly<TestType>::Vector sol_integ(4); sol_integ << c, 1.0, -3.0/2.0, 2.0/3.0;
