...
TEMPLATE_TEST_CASE("GCD", "[template]", float, double) {
  ...
  SECTION("Test 1") {
    ...
    typename Poly<TestType>::Vector sol_gcd(1); sol_gcd << 1; // gcd(x) = 1
  ...
  SECTION("Test 2") {
    ...
    typename Poly<TestType>::Vector sol_gcd(1); sol_gcd << 1.0; // gcd(x) = 1
    REQUIRE(gcd.coeffs().isApprox(sol_gcd));
    REQUIRE(gcd.degree() == 0);
  }
  ...
  SECTION("Test 3") {
    ...
    typename Poly<TestType>::Vector sol_gcd(2); sol_gcd << 1, -1; // gcd(x) = 1 - x
    REQUIRE(gcd.coeffs().isApprox(sol_gcd));
