...
namespace Sturm {
   ...
   *
   * This class implements a polynomial of the form \f$ p(x) = \sum_{i=0}^n a_i
   * x^i \f$.
  ...
  class Poly : public Eigen::Vector<Real, Eigen::Dynamic> {
   public:
    using Vector =
        Eigen::Vector<Real, Eigen::Dynamic>; /**< Vector of real numbers. */
   ...
   private:
    ...
    /**
     * \brief Access to the Eigen class.
   ...
   public:
    /**
     * \brief Class constructor from a vector.
     */
    Poly() : m_order(0) {}
    ...
    /**
     * \brief Access to the Eigen const class.
    ...
    /**
     * \brief Set the order of the polynomial.
     * \param[in] order Order of the polynomial.
     */
    ...
    /**
     * \brief Set the degree of the polynomial.
    ...
    /**
     * \brief Set the polynomial to a scalar value.
    ...
    /**
     * \brief Set the polynomial to a monomial with a given coefficient.
    ...
    /**
     * \brief Get the order of the polynomial.
    ...
    /**
     * \brief Evaluate the derivative of the polynomial at a given point.
    ...
    /**
     * \brief Evaluate the polynomial and its derivative at a given point.
    ...
    /**
     * \brief Get the leading coefficient of the polynomial.
    ...
    /**
     * \brief Compute the integral of the polynomial.
    ...
    /**
     * \brief Compute the integral of the polynomial with a given constant.
     * \param[out] res Polynomial representing the integral.
    ...
    /**
     * \brief Normalize the polynomial.
     *
    ...
    /**
     * \brief Purge small coefficients.
     *
     * Purge (set to 0) coefficients that are smaller than or equal to a given
     * epsilon.
    ...
    /**
     * \brief Adjust the degree of the polynomial and remove leading zeros.
     *
     * Adjust the degree of the polynomial to ensure the leading coefficient is
     * non-zero.
     */
    void adjust_degree() {
    ...
    /**
     * \brief Count the number of sign variations.
     *
    ...
    /**
     * \brief Normalize the polynomial to monic form.
     *
     * Scales all coefficients such that the leading coefficient becomes 1.
     * After this operation, the polynomial satisfies
     * \f$ p(x) = x^n + \sum_{i=0}^{n-1} a_i x^i \f$,
     * where \f$ n \f$ is the polynomial degree.
     *
     * \pre The leading coefficient must be non-zero.
     */
    void make_monic() {
    ...
    /**
     * \brief Define the assignment with another polynomial.
    ...
    /**
     * \brief Define the negation of the polynomial.
    ...
    /**
     * \brief Define the addition with another polynomial.
    ...
    /**
     * \brief Define the subtraction with another polynomial.
    ...
    /**
     * \brief Define the multiplication with another polynomial.
    ...
    /**
     * \brief Define the addition with a scalar.
    ...
    /**
     * \brief Define the subtraction with a scalar.
    ...
    /**
     * \brief Define the multiplication with a scalar.
  ...
  /**
   * \brief Define the sum between two polynomials.
  ...
  /**
   * \brief Define the sum between a polynomial and a scalar.
  ...
  /**
   * \brief Define the difference between two polynomials.
  ...
  /**
   * \brief Define the difference between a scalar and a polynomial.
   * \param[in] p Polynomial to subtract.
   * \param[in] s Scalar to subtract.
   * \return The operation result.
   * \tparam Real Scalar number type.
  ...
  /**
   * \brief Define the multiplication between two polynomials.
  ...
  /**
   * \brief Define the multiplication between a scalar and a polynomial.
   * \param[in] p Polynomial to multiply.
   * \param[in] s Scalar to multiply.
   * \return The operation result.
   * \tparam Real Scalar number type.
  ...
  /**
   * \brief Divide the polynomial.
   *
   * Divide the polynomial \f$ p_1(x) \f$ by \f$ p_2(x) \f$ with remainder \f$
   * r(x) \f$ and quotient \f$ q(x) \f$.
  ...
  /**
   * \brief Print the polynomial on an output stream.
